...
const Section3 = () => {
  ...
  return (
    <section className="container mx-auto px-6 md:px-20 py-10">
      ...
      >
        {data?.map((value, i) => {
          return <SwiperSlide key={i}>{<Post data={value} />}</SwiperSlide>;
        })}
      </Swiper>
    </section>
  );
    ...
    <div className="grid">
      ...
      <div className="info flex justify-center flex-col py-4">
        ...
        <div className="title">
          <Link
            href={`/posts/${id}`}
            className="text-3 text-3xl md:text-4xl  font-bold text-gray-800 hover:text-gray-500"
          >
            {title}
          </Link>
        </div>
        <p className="text-gray-500 py-3 lg:pr-28">{subtitle}</p>
